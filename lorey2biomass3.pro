...
;for lorey2biomass3, copied from lorey2biomass2 and update Amazon ecozone equations


;in_type definitions, should be type byte. 3 digit number


Pro tropic_addsap, agb_array
	sap_array = -0.01072 * agb_array + 16.8
	index = where(sap_array lt 0, count)
	if (count gt 0) then sap_array[index] = 0
	agb_array = agb_array + sap_array
End

Pro asia_addsap, agb_array
	sap_array = -0.034396 * agb_array + 24.263
	index = where(sap_array lt 0, count)
	if (count gt 0) then sap_array[index] = 0
	agb_array = agb_array + sap_array
End
...
;	30		-	class 40
		30:	begin
			agb_array[*] = 0.6011 * (lorey_array^1.894) < MAX_AGB
			tropic_addsap, agb_array; added sapling
			index = where((agb_array lt 125), count, complement=index2, ncomplement=count2)
			if (count gt 0) then bgb_array[index] = 0.205 * agb_array[index]
			if (count2 gt 0) then bgb_array[index2] = 0.235 * agb_array[index2]
		end
...
;	70		-	class 40
		70:	begin
			agb_array[*] = 0.2788 * (lorey_array^2.12)  < MAX_AGB
			tropic_addsap, agb_array; added sapling
			index = where((agb_array lt 125), count, complement=index2, ncomplement=count2)
			if (count gt 0) then bgb_array[index] = 0.205 * agb_array[index]
			if (count2 gt 0) then bgb_array[index2] = 0.235 * agb_array[index2]
...
;	90		-	class 40 (tropical rain forest)
		90:	begin
			...
			asia_addsap, agb_array   ;Add sapling
...
;	110		-	class 40
		110:	begin
			agb_array[*] = 0.39194 * (lorey_array^2.1506) < MAX_AGB
			asia_addsap, agb_array; using sapling from SEA
			index = where((agb_array lt 125), count, complement=index2, ncomplement=count2)
			if (count gt 0) then bgb_array[index] = 0.205 * agb_array[index]
			if (count2 gt 0) then bgb_array[index2] = 0.235 * agb_array[index2]
		end
...
;South America Ecozones
		201: begin
			agb_array[*] = 3.3986 * (lorey_array^1.326) < MAX_AGB
			tropic_addsap, agb_array ; Add Sapling
		...
		202: begin
			agb_array[*] = 2.9077 * (lorey_array ^ 1.3257) < MAX_AGB
			tropic_addsap, agb_array ; Add Sapling
			index = where((agb_array lt 125), count, complement=index2, ncomplement=count2)
			if (count gt 0) then bgb_array[index] = 0.205 * agb_array[index]
			if (count2 gt 0) then bgb_array[index2] = 0.235 * agb_array[index2]
		...
		203: begin
			agb_array[*] = 0.2163 * (lorey_array ^ 2.1835) < MAX_AGB
			tropic_addsap, agb_array
			index = where((agb_array lt 125), count, complement=index2, ncomplement=count2)
			if (count gt 0) then bgb_array[index] = 0.205 * agb_array[index]
			if (count2 gt 0) then bgb_array[index2] = 0.235 * agb_array[index2]
		end
		204: begin
			agb_array[*] = 3.1721 * (lorey_array ^ 1.3257) < MAX_AGB
			tropic_addsap, agb_array
			index = where((agb_array lt 125), count, complement=index2, ncomplement=count2)
			if (count gt 0) then bgb_array[index] = 0.205 * agb_array[index]
			if (count2 gt 0) then bgb_array[index2] = 0.235 * agb_array[index2]
		...
		206: begin
			agb_array[*] = 0.23065 * (lorey_array ^ 2.217) < MAX_AGB
			tropic_addsap, agb_array
			index = where((agb_array lt 125), count, complement=index2, ncomplement=count2)
			if (count gt 0) then bgb_array[index] = 0.205 * agb_array[index]
			if (count2 gt 0) then bgb_array[index2] = 0.235 * agb_array[index2]
		...
		208: begin
			agb_array[*] = 2.7122 * (lorey_array ^ 1.3257) < MAX_AGB
			tropic_addsap, agb_array
			index = where((agb_array lt 125), count, complement=index2, ncomplement=count2)
			if (count gt 0) then bgb_array[index] = 0.205 * agb_array[index]
			if (count2 gt 0) then bgb_array[index2] = 0.235 * agb_array[index2]
		...
		209: begin
			agb_array[*] = 0.5009 * (lorey_array ^ 1.894) < MAX_AGB
			tropic_addsap, agb_array
			index = where((agb_array lt 125), count, complement=index2, ncomplement=count2)
			if (count gt 0) then bgb_array[index] = 0.205 * agb_array[index]
			if (count2 gt 0) then bgb_array[index2] = 0.235 * agb_array[index2]
		...
		211: begin
			agb_array[*] = 0.57104 * (lorey_array ^ 1.894) < MAX_AGB
			tropic_addsap, agb_array
			index = where((agb_array lt 125), count, complement=index2, ncomplement=count2)
			if (count gt 0) then bgb_array[index] = 0.205 * agb_array[index]
			if (count2 gt 0) then bgb_array[index2] = 0.235 * agb_array[index2]
...
PRO lorey2biomass3, in_lorey, in_type, agb_array, bgb_array
